...
SLURM_DIR=BASE_data+"sim_triplehets/"

OUT_DIR=BASE_data+"data_sim_triplehets/"
...
def makeCommand(vcf,p1,p2,ngenes,nvars,reads,recomb,theta,mcmc):
    ...
    trio_cmd = "LAMBDA=1.2; AFFECTED=0.5; " +\
                "code/refactored.py -s stan.out /hpc/home/sh700/ase_models/Refactored_triphets " + \
                phased + " " + str(mcmc) + " " + \
...
for var in VARS_PER_GENE:
    addCommands(slurm, var, 30, .5)

for reads in READS_PER_SITE:
    addCommands(slurm, 10, reads, .5)

for thet in THETA:
    addCommands(slurm, 10, 30, thet)
